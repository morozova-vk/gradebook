        ...
        <div class="mb-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 p-5">
                    <div class="text-center"><div class="text-2xl font-bold text-indigo-600">${stats.count}</div><div class="text-xs text-slate-500 mt-1">Учеников</div></div>
...
function renderStatsGraphs() {
    ...
    const classContainer = document.getElementById('graphClassContainer');
    ...
    if (clsVal && subjVal) {
        ...
        if (data && data.students && data.students.length > 0) {
            ...
            classContainer.innerHTML = `
                <div class="w-full">
                    <div class="flex justify-between items-center mb-4 flex-wrap gap-2">
                        <h3 class="text-xl font-bold text-slate-800">${escapeHTML(clsVal)} — ${escapeHTML(subjVal)}</h3>
                        <div class="flex gap-4 text-sm">
                            <span class="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full">📊 Средний: ${stats.avg}</span>
                            <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full">👥 Учеников: ${stats.count}</span>
                            <span class="px-3 py-1 bg-amber-100 text-amber-700 rounded-full">📝 Оценок: ${stats.totalGrades}</span>
                        </div>
                    </div>
                    <div class="w-full" style="height: 400px;">
                        <canvas id="chart_class" style="width:100% !important; height:100% !important;"></canvas>
                    </div>
                </div>`;
            setTimeout(() => {
                const canvas = document.getElementById('chart_class');
                if (canvas) {
                    const ctx = canvas.getContext('2d');
                    if (chartInstances['chart_class']) {
                        try { chartInstances['chart_class'].destroy(); } catch(e) {}
                    }
                    chartInstances['chart_class'] = new Chart(ctx, {
                        type: 'bar',
                        data: {
                            labels: ['5 (отлично)', '4 (хорошо)', '3 (удовлетворительно)', '2 (неудовлетворительно)'],
                            datasets: [{
                                label: 'Количество оценок',
                                data: [stats.distribution[5] || 0, stats.distribution[4] || 0, stats.distribution[3] || 0, stats.distribution[2] || 0],
                        ...
                        options: {
                            ...
                            plugins: {
                                legend: { 
                                    position: 'top',
                                    labels: { font: { size: 13 }, usePointStyle: true, boxWidth: 10 }
                                },
                            ...
                            scales: { 
                                y: { 
                                    ...
                                    grid: { color: '#e2e8f0' },
                                    title: { display: true, text: 'Количество оценок', font: { size: 12 } }
                                },
                                x: { 
                                    ticks: { font: { size: 12 } },
                                    grid: { display: false }
            ...
            classContainer.innerHTML = '<div class="flex items-center justify-center h-64 text-slate-400 bg-slate-50 rounded-2xl">📊 Нет данных для выбранного класса и предмета</div>';
        }
    } else {
        classContainer.innerHTML = '<div class="flex items-center justify-center h-64 text-slate-400 bg-slate-50 rounded-2xl">📊 Выберите класс и предмет для просмотра графика</div>';
    }
    
    // График 2: все классы по выбранному предмету
    const overallContainer = document.getElementById('graphOverallContainer');
    ...
    if (overallSubjVal) {
        ...
        if (allGrades.length > 0) {
            ...
            const avg = (allGrades.reduce((a,b) => a+b, 0) / total).toFixed(2);
            overallContainer.innerHTML = `
                <div class="w-full">
                    <div class="flex justify-between items-center mb-4 flex-wrap gap-2">
                        <h3 class="text-xl font-bold text-slate-800">🌍 ${escapeHTML(overallSubjVal)} (все классы)</h3>
                        <div class="flex gap-4 text-sm">
                            <span class="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full">📊 Средний: ${avg}</span>
                            <span class="px-3 py-1 bg-amber-100 text-amber-700 rounded-full">📝 Всего оценок: ${total}</span>
                        </div>
                    </div>
                    <div class="w-full" style="height: 400px;">
                        <canvas id="chart_overall" style="width:100% !important; height:100% !important;"></canvas>
                    </div>
                </div>`;
            setTimeout(() => {
                const canvas = document.getElementById('chart_overall');
                if (canvas) {
                    const ctx = canvas.getContext('2d');
                    if (chartInstances['chart_overall']) {
                        try { chartInstances['chart_overall'].destroy(); } catch(e) {}
                    }
                    chartInstances['chart_overall'] = new Chart(ctx, {
                        type: 'bar',
                        data: {
                            labels: ['5 (отлично)', '4 (хорошо)', '3 (удовлетворительно)', '2 (неудовлетворительно)'],
                            datasets: [{
            ...
            overallContainer.innerHTML = '<div class="flex items-center justify-center h-64 text-slate-400 bg-slate-50 rounded-2xl">📊 Нет данных для выбранного предмета</div>';
        }
    } else {
        overallContainer.innerHTML = '<div class="flex items-center justify-center h-64 text-slate-400 bg-slate-50 rounded-2xl">📊 Выберите предмет для просмотра общей статистики</div>';
    }
}
